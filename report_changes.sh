...
# Load my aliases
source ~/.oh-my-zsh/custom/my_aliases.zsh > /dev/null 2>&1

#Specify the folder where the script is located
root_directory=$(dirname "$(realpath "$0")")

# Specify the source directory
source_directory="$root_directory/MASTERBRANCH/"
...
copyFile() {
  ...
  if [[ -f "$source_file" ]] && [[ -f "$target_file" ]]; then
    ...
    # Replace 'projectmaster' with '{{projectName}}' in the target file
    sed -i '' 's/projectmaster/{{projectName}}/g' "$target_file"
